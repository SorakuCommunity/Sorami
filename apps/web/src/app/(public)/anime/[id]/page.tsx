...
export default async function AnimePage({ params, searchParams }: AnimeDetailPageProps) {
  ...
  const slug = slugify(anime.titleEnglish || anime.title);
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0F]">
      ...
      <Container className="-mt-[220px] md:-mt-[300px] relative z-10">
            ...
            <>
              <div className="grid gap-3">
                {paginatedEpisodes.map((ep) => (
                  <Link
                    key={ep.number}
                    href={`/watch/${id}/${slug}?ep=${ep.number}`}
        ...
        {/* Related anime */}
        <Section title="Related Anime" seeAllLink={`/anime/${id}/related`}>
          {relatedAnime.map((a) => (
