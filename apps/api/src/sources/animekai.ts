...
@Injectable()
export class AnimeKai implements Provider {
  ...
  async search(query: string, page = 1): Promise<SearchResult> {
    const { data } = await this.http.get(`/search?q=${encodeURIComponent(query)}&page=${page}`);
  ...
  async stream(episodeId: string): Promise<StreamResult | null> {
    try {
      const { data } = await this.http.get(episodeId.startsWith('http') ? episodeId : `/watch/${episodeId}`);
      const $ = cheerio.load(data);
      const sources: any[] = [];
      $('source').each((_, el) => {
        const src = $(el).attr('src');
        if (src) sources.push({ url: src, quality: $(el).attr('size') || '720p', isM3U8: src.includes('.m3u8') });
      });
      return { sources, subtitles: [] };
    } catch { return null; }
  }

  async recent(page = 1): Promise<SearchResult> {
    const { data } = await this.http.get(`/recently-added?page=${page}`);
    return { results: this.parseEntries(data) };
  }
