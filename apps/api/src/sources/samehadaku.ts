...
const BASE = 'https://samehadaku.mba';

@Injectable()
export class Samehadaku implements Provider {
  ...
  async info(id: string): Promise<AnimeEntry | null> {
    try {
      ...
      return {
        ...
        image: $('.post-thumbnail img, .thumb img').first().attr('src'),
        description: $('.entry-content p, .sinops p').first().text().trim(),
        genres: ([] as string[]).concat(...$('.genre-info a, .genres a').map((_, e) => $(e).text().trim()).get()),
        status: $('.status-info, .info-item:contains("Status")').text().replace('Status', '').trim(),
      };
    } catch { return null; }
  }
