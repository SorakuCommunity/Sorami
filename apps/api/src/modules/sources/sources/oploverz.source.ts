...
const BASE_URL = 'https://oploverz.bond';

@Injectable()
export class OploverzSource implements AnimeSource {
  readonly name = 'Oploverz';
  readonly type = 'primary' as const;
  readonly priority = 85;
  private readonly logger = new Logger(OploverzSource.name);
  ...
  async getAnime(id: string): Promise<SourceAnimeDetail | null> {
    try {
      ...
      const posterUrl = $('.post-thumbnail img, .thumb img, .series-img img').first().attr('src');
      const synopsis = $('.entry-content p, .desc p').first().text().trim();
      const genres: string[] = [];
      $('.genre-info a, .genres a, .genre a').each((_, el) => { genres.push($(el).text().trim()); });
      const status = $('.status-info, .info-item:contains("Status")').text().replace('Status', '').trim();
      const scoreText = $('.rating, .score').first().text().trim();
      const score = scoreText ? parseFloat(scoreText) : undefined;
      const episodes: SourceEpisode[] = [];
      $('.episodelist li, .listeps li, .ep-list a').each((_, el) => {
  ...
  async resolveEpisodeUrl(animeSlug: string, episodeNumber: number): Promise<string | null> {
    try {
      ...
      const $ = cheerio.load(data);
      let episodeUrl = '';
      $('.episodelist li a, .listeps li a, .ep-list a').each((_, el) => {
