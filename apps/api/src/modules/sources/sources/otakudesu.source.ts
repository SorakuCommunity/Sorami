...
@Injectable()
export class OtakudesuSource implements AnimeSource {
  readonly name = 'Otakudesu';
  readonly type = 'primary' as const;
  readonly priority = 80;
  private readonly logger = new Logger(OtakudesuSource.name);
  ...
  async getAnime(id: string): Promise<SourceAnimeDetail | null> {
    try {
      ...
      const posterUrl = $('.post-thumbnail img, .thumb img, .series-thumb img').first().attr('src');
      ...
      $('.episodelist ul li, .listeps li, .episode-list a').each((_, el) => {
  ...
  async resolveEpisodeUrl(animeSlug: string, episodeNumber: number): Promise<string | null> {
    try {
      ...
      const $ = cheerio.load(data);
      let episodeUrl = '';
      $('.episodelist ul li a, .listeps li a, .episode-list a').each((_, el) => {
