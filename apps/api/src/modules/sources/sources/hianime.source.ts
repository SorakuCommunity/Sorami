...
export class HiAnimeSource implements AnimeSource {
  ...
  async search(query: string): Promise<SourceAnimeEntry[]> {
    const { data } = await this.axios.get(`/search?keyword=${encodeURIComponent(query)}`);
    return this.parseEntries(data);
  }

  async getAnime(id: string): Promise<SourceAnimeDetail | null> {
    try {
      const { data } = await this.axios.get(`/watch/${id}`);
      const $ = cheerio.load(data);
      const title = $('.anisc-info .heading, h1').first().text().trim();
      const posterUrl = $('.anisc-poster img, .thumb img').first().attr('src');
      const synopsis = $('.description, .desc').first().text().trim();
      const genres: string[] = [];
      $('.genre a, .genres a').each((_, el) => { genres.push($(el).text().trim()); });
      const status = $('.status span:last, .info-item:contains("Status") span').text().trim() || 'UNKNOWN';
      const scoreText = $('.rating span:last').text().trim();
      const score = scoreText ? parseFloat(scoreText) : undefined;
      const episodes: SourceEpisode[] = [];
      $('.episodes-list a, .eps-list a').each((_, el) => {
        const epNum = parseInt($(el).text().match(/\d+/)?.[0] || '', 10);
        const href = $(el).attr('href') || '';
        if (!isNaN(epNum)) episodes.push({ number: epNum, episodePageUrl: href, videoUrls: {} });
      });
      return { title, slug: id, synopsis, posterUrl, genres, status, score, episodes, source: this.name };
    } catch {
      return null;
    }
  }
  ...
  async resolveEpisodeUrl(animeSlug: string, episodeNumber: number): Promise<string | null> {
    try {
      const { data } = await this.axios.get(`/watch/${animeSlug}`);
      const $ = cheerio.load(data);
      let episodeUrl = '';
      $('.episodes-list a, .eps-list a').each((_, el) => {
  ...
  async getPopular(page = 1): Promise<SourceAnimeEntry[]> {
    const { data } = await this.axios.get(`/most-popular?page=${page}`);
    return this.parseEntries(data);
  }
